...
namespace DungeonRunners.Networking.EntitySynchInfo
{
    ...
    public readonly struct ResolvedEntitySynchInfo
    {
        ...
        public ResolvedEntitySynchInfo(EntitySynchInfoPayload payload, uint ownerEntityId, uint componentId, byte subtype, float clientNow, string reason, string provenance, uint validationCutoffTick = 0, float validationCutoffTime = -1f, string runtimeInstanceKey = null, uint schedulerTick = 0, bool subEntityPhase = false, int rngPos = -1, string hpMutationSource = null)
        {
            Payload = payload;
        ...
        public static ResolvedEntitySynchInfo FromHP(uint ownerEntityId, uint componentId, byte subtype, uint hpWire, float clientNow, string reason, string provenance)
        {
            return new ResolvedEntitySynchInfo(EntitySynchInfoPayload.FromHP(hpWire), ownerEntityId, componentId, subtype, clientNow, reason, provenance);
        }

        public override string ToString()
        {
            return $"{Payload} owner={OwnerEntityId} component={ComponentId} sub=0x{Subtype:X2} clientNow={Now:F3} cutoffTick={ValidationCutoffTick} cutoffTime={ValidationCutoffTime:F3} runtime='{RuntimeInstanceKey}' schedulerTick={SchedulerTick} subentity={SubEntityPhase} rngPos={RngPos} hpMutation='{HpMutationSource}' reason={Reason} provenance={Provenance}";
        }
    }
}
