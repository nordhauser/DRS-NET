...
{
    public sealed class NativePackageCatalog
    {
        private static NativePackageCatalog _instance;
        public static NativePackageCatalog Instance => _instance ??= new NativePackageCatalog();

        private readonly List<NativePackageTextDocument> _gcDocuments = new List<NativePackageTextDocument>();
        private readonly Dictionary<string, NativePackageTextDocument> _gcByName = new Dictionary<string, NativePackageTextDocument>(StringComparer.OrdinalIgnoreCase);
        ...
        public IEnumerable<NativePackageTextDocument> GcTextDocuments => _gcDocuments;
        ...
        public bool LoadFromAssets()
        {
            ...
            {
                Debug.LogError($"[NATIVE-PACKAGE-CATALOG] source=missing path='{root}'");
                return false;
            }
            ...
            Debug.LogError($"[NATIVE-PACKAGE-CATALOG] source=NativeSidecar loaded={IsLoaded} gcTextDocs={_gcDocuments.Count}/{ManifestGcTextDocs} path='{root}' runtimePkgDependency=false");
            return IsLoaded;
        }

        public IEnumerable<NativePackageTextDocument> EnumerateGcTextDocuments(string searchPattern = "*.gc")
        {
        ...
        public bool TryGetGcText(string nameOrPath, out NativePackageTextDocument document)
        {
            document = null;
        ...
        private void LoadGcTextDocuments(string path)
        {
            foreach (string line in ReadJsonLines(path))
            {
                if (string.IsNullOrWhiteSpace(line))
                    continue;
                var document = new NativePackageTextDocument
                {
        ...
        private void Register(NativePackageTextDocument document)
        {
        ...
        private void AddName(string name, NativePackageTextDocument document)
        {
        ...
        private static string ResolveFileName(NativePackageTextDocument document)
        {
    ...
    public sealed class NativePackageTextDocument
    {
        public int EntryId;
