...
{
    public static class NativeFallbackAudit
    {
        private static bool _startupCoverageLogged;
        ...
        private static void ReportClassDamageModCoverage()
        {
            ...
            foreach (string classBase in classes)
            {
                ...
                string rangedSource = hasRanged ? "gc" : "native-default";
                string meleeSource = hasMelee ? "gc" : "native-default";
                string skillSource = hasSkill ? "gc" : "native-default";
                ...
                Debug.LogError($"[AUTHORED-COVERAGE] area=class-damage class={classBase} resolved={node != null} rangedMod={rangedMod:F3} rangedSource={rangedSource} meleeMod={meleeMod:F3} meleeSource={meleeSource} skillMod={skillMod:F3} skillSource={skillSource} native=HeroDesc-class-stat-default");
            }
        }

        private static void ReportChestGeneratorCoverage()
        {
            Debug.LogError("[AUTHORED-COVERAGE] area=chest-generators auditedSlots=1-10 nativeSlots=1-5 runtimeSlots=1-2 unsupportedSlots=6-10 goldGenerators=partial tokenGenerators=partial source=server-startup-report");
        }
