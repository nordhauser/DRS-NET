...
namespace DungeonRunners.Networking
{
    ...
    {
        public const uint NativeTickIntervalMs = 0x21;
        public const uint NativeCadence = 0x0B;
        ...
        public void Reset(string instanceKey, string source = null)
        {
            ...
            LastCutoffTime = -1f;
            LastCutoffTick = 0;
            Debug.LogError($"[CLIENT-ENTITY-SCHED] reset instance='{RuntimeInstanceKey}' source={source ?? "unknown"} interval=0x{NativeTickIntervalMs:X} cadence=0x{NativeCadence:X}");
        }
        ...
        public void ObserveSuffixCutoff(string instanceKey, uint cutoffTick, float cutoffTime, bool subEntityPhase, string phase, string source = null)
        {
            ...
            UpdateNumber++;
            CadenceCounter = NativeCadence == 0 ? 0 : (CadenceCounter + 1u) % NativeCadence;
            Debug.LogError($"[CLIENT-ENTITY-SCHED] instance='{RuntimeInstanceKey}' update={UpdateNumber} cadence={CadenceCounter}/0x{NativeCadence:X} tick={SchedulerTick} cutoff={cutoffTick}@{cutoffTime:F3} subentity={SubEntityPhase} phase={LastPhase} source={source ?? "unknown"}");
        }
    }
}
